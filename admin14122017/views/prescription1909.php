...
<?php $data['page']='five'; $data['title']='Prescription Verification'; $this->load->view('layout/header',$data);?>
...
{
?>           
  <img id="refill<?php echo $key->id; ?>" data-toggle="modal" data-target="#rf<?php echo $key->id; ?>" src="<?php echo base_url('precriptionImage/'.$key->image);?>" width="60" height="60" style="cursor:pointer; padding:5px; border:1px solid;">
          ...
          <div class="modal-footer" style="background: rgba(0, 0, 0, 0.92);opacity: 1.1;border:none;">
            <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                                                  ...
                                                  <td><php echo $list->prescription_scan ?></td-->
                                                   <td><?php echo date('Y-m-d / h:i A',strtotime($list->time_stamp));?></td>

                                                   <?php if($this->session->userdata('status')=='admin'){ ?>

                                                   <td>
                                                   <?php if($list->pharmacist_id==0){?>
                                                    <a href="<?php echo site_url('PrescriptionControler/assign_pharmacist?pres_id='.$list->id.'&ph_name='.$list->pharmacy_name); ?>" style="text-decoration:none;">
                                                    <input type="button" class="btn btn-success" value="Assign RPh" style="background-color:#f37022; color:white; "></a>
                                                    <?php } else{ ?>

                                                    <a href="<?php echo site_url('PrescriptionControler/change_pharmacist?pres_id='.$list->id.'&ph_id='.$list->pharmacist_id.'&ph_name='.$list->pharmacy_name); ?>" style="text-decoration:none;">
