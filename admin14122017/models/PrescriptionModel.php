...
class PrescriptionModel extends CI_Model {
	...
	{		
		// $this->db->select('prescription.pharmacist_id,prescription.user_id,prescription.id,prescription.insurance_image,prescription_scan,prescription.time_stamp,registration.id as user_id,registration.first_name,last_name,email,user_pharmacy.pharmacy_name,city,cross_street,contactNo');
		$this->db->select('prescription.pharmacist_id,pharmacy_id,prescription.user_id,prescription.id,prescription.insurance_image,prescription_scan,prescription.time_stamp,registration.id as user_id,registration.first_name,last_name,email');
	...
	{
		$this->db->select('prescription.pharmacist_id,prescription.user_id,prescription.id,prescription.insurance_image,prescription_scan,prescription.time_stamp,registration.first_name,last_name,email,user_pharmacy.pharmacy_name,city,cross_street,contactNo');
		$this->db->from('prescription');
		$this->db->join('registration', 'prescription.user_id = registration.id');
		$this->db->join('user_pharmacy','prescription.pharmacy_id = user_pharmacy.id');
	...
	public function user_prescriptionImage($prescription_id)
	{
		return $this->db->get_where('prescription_images',array('prescription_id'=>$prescription_id))->result();
	}
	...
	{				
		$this->db->select('prescription.pharmacist_id,pharmacy_id,prescription.user_id,prescription.id,prescription.insurance_image,prescription_scan,prescription.time_stamp,registration.first_name,last_name,email');
		$this->db->from('prescription');
		$this->db->join('registration', 'prescription.user_id = registration.id');
		// $this->db->join('user_pharmacy','prescription.pharmacy_id = user_pharmacy.id');
...
// -------------------------------------------------------------------------------------------------------------
	...
	public function renew_prescription()
	{
		...
		$this->db->select('prescription_medicine.*,registration.id as user_id,registration.first_name,last_name,email,prescription.pharmacy_id');
...
// -------------------------------------------------------------------------------------------------------------
	...
	public function add_time($medicine_id,$prescription_id,$time,$timestype,$endDate)
    {
        $data = array();
        ...
        foreach($daterange as $dosedate)
        {
            ...
            for($ts=0; $ts<count($time); $ts++)
            {
            	$data[] = array(
                "medicine_id"       => $medicine_id,
                "prescription_id"   => $prescription_id,
                "time"              => $time[$ts],
                "time_type"         => $timestype[$ts],
                "dose_date"         => $dose_date
                 );
            }
    ...
    public function add_renewtime($medicine_id,$prescription_id,$time,$timetype,$begin_date,$endDate)
    {
        $data = array();
        //Date between two dates
        $begin      = new DateTime($begin_date);
...
// -------------------------------------------------------------------------------------------------------------
    ...
    public function update_time($timedata,$pres_id,$med_id)
	{
		...
		if($timedata['newdays']>$timedata['olddays'])
		{
	        ...
	        foreach($daterange as $dosedate)
	        {
	            ...
	            for($t=0; $t<$timecount; $t++)
	            {
	                ...
	                "time_type"         => $timedata['time_type'][$t],
	                "dose_date"         => $dose_date,
	                "status"			=>'not'
	                 );
	            } 
		...
		{	
	        ...
	        foreach($daterange as $dosedate)
	        {
	        	...
	        	for($ts=0; $ts<count($timedata['time']); $ts++)
	            {
	            	$datat[] = array(
	                "time"              => $timedata['time'][$ts],
	                "time_type"         => $timedata['time_type'][$ts],
	                "dose_date"         => $dose_date
	                 );
	            }
			...
			for($i=0; $i<$NoTimes; $i++)
			{
				...
				$this->db->set('time_type',$datat[$i]['time_type']);
	...
	{
		$this->db->select('id,time,time_type,dose_date');
	...
	public function UserMedicine($user_id)
	{
		$query=$this->db->get_where('prescription_medicine',array('user_id'=>$user_id));
	...
	public function MedicineHistory($medicine_id)
	{
