...
function buildMessageHTML(messageText, messageSenderId, messageDateSent, attachmentFileId, messageId, status){
  var messageAttach;
  if(attachmentFileId){
      messageAttach = '<a href="#" data-toggle="modal" data-target="#qbimageModal"><img onclick="changeIt(this)" src="' + QB.content.publicUrl(attachmentFileId) + '/' + '/download.xml?token='+token+'" alt="attachment" class="attachments img-responsive" /></a>';
  }

	var isMessageSticker = stickerpipe.isSticker(messageText);

  var delivered = '<img class="icon-small" src="http://18.221.70.138/medical/assest/chat/images/delivered.jpg" alt="" id="delivered_'+messageId+'">';
  var read = '<img class="icon-small" src="http://18.221.70.138/medical/assest/chat/images/read.jpg" alt="" id="read_'+messageId+'">';
