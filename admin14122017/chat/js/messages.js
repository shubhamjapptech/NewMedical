    ...
    QB.chat.message.list(params, function(err, messages) {
      if (messages) {
        ...
        messages.items.forEach(function(item, i, arr) {
          ...
          //var messageSenderName = item.full_name;        
          //var messageDateSent = new Date(item.date_sent*1000); 
        var date = new Date(item.date_sent*1000);
        var hours = date.getHours() > 12 ? date.getHours() - 12 : date.getHours();
        var am_pm = date.getHours() >= 12 ? "PM" : "AM";
        hours = hours < 10 ? "0" + hours : hours;
        var minutes = date.getMinutes() < 10 ? "0" + date.getMinutes() : date.getMinutes();
        var messageDateSent = hours + ":" + minutes +" " + am_pm;



          var messageSenderLogin = getUserLoginById(messageSenderId);
          //var messageSenderLogin = dialog.name;
...
function clickSendAttachments(inputFile) {
    QB.content.createAndUpload({
        ...
        if(err) {
            ...
            var uploadedFile = response;
            var fileUid = uploadedFile.uid;
            var publicImageUrl = QB.content.publicUrl(fileUid);
            //alert(publicUrl);
            //console.log('image_url'+publicUrl);


            sendMessage("[attachment]", uploadedFile.id,publicImageUrl);
...
function sendMessage(text, attachmentFileId,publicImageUrl) {
    ...
    if(attachmentFileId !== null){
        msg['extension']['attachments'] = [{id: attachmentFileId, type: 'photo',url:publicImageUrl}];
    }
...
function showMessage(userId, msg, attachmentFileId) 
{
    ...
    hours = hours < 10 ? "0" + hours : hours;
    var minutes = date.getMinutes() < 10 ? "0" + date.getMinutes() : date.getMinutes();
    var msgtime = hours + ":" + minutes +" " + am_pm;
     
    var messageHtml = buildMessageHTML(msg.body, userLogin, msgtime, attachmentFileId, msg.id);
