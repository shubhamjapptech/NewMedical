...
class PrescriptionControler extends CI_Controller {
    ...
    {
        $u_id=$_GET['u_id']; //User_id
        $p_id=$_GET['p_id'];
        $pres_id=$_GET['pres_id'];    //Prescription Id
        if($this->input->post('submit'))
        {
            ...
            $Dailytake      = $total_medicine/($total_day*$times);      
            ...
            $basic_data = array(
                    ...
                    "prescriber_number" => $pres_number,
                    "prescriber_address"=> $pres_add,
                    "total_refill"      => $refill,
                    "refill_remain"     => $refill,
            ...
            { 
                $this->PrescriptionModel->add_time($medicine_id,$prescription_id,$TabTimes,$timetype,$endDate);        
                $p_id=$this->session->userdata('id');
                $data=new stdClass();
                $data->u_id=$u_id;
                $data->p_id=$p_id;
                $data->pres_id=$pres_id;
                $data->images  = $this->PrescriptionModel->user_prescriptionImage($pres_id);
                $data->med=1;
                $data->success=1;
                $data->message="Prescription has been added successfully";
        ...
        else
        {
            $data=new stdClass();
            $data->med=0;
            $data->u_id = $u_id;  //User_id
            $data->p_id = $p_id;
            $data->pres_id = $pres_id;    //Prescription id
            $data->images  = $this->PrescriptionModel->user_prescriptionImage($pres_id);
            $this->load->view('add_prescription',$data);
        }
    }
...
// -------------------------------------------------------------------------------------------------------------
    ...
    public function add_renew_prescription()
    {
        ...
        if($this->input->post('update'))
        {
            ...
            {
                //echo "today is not greater";
                $startdate = $previousendDate;
                $end       = strtotime($previousendDate.'+'.$newdays.' day');
                $enddate   = date('d-m-Y',$end);
                $remain    = $remain+$quantity;   //remain_medicine
                $quantity  = $quantity+$preQuantity;   //total_medicine
            ...
            if($response!=false)
            {
                $this->PrescriptionModel->add_renewtime($med_id,$pres_id,$time,$timetype,$startdate,$enddate);
                $data=new stdClass();
...
// -------------------------------------------------------------------------------------------------------------
    ...
    public function update_prescription()
    {
        ...
        {   
            ...
            $response=$this->PrescriptionModel->update_presctiption($data1,$timedata,$pres_id,$medicine_id);
            // $response=$this->PrescriptionModel->update_presctiption($timedata);
            if($response!=false)
            {
                // $data=new stdClass();
                ...
                $data['success']=1;
                $data['message']="Prescription has been Updated successfuly ";
                $data['list']=$record;
                $data['mtime']=$rec_time;
    ...
    public function show_prescription()
    {
        ...
        if($pageFrom=='two'){$data->user_id =$_GET['user_id']; }
    ...
    public function UserMedicine()
    {
        ...
        $data    = new stdClass();
        $data->page= $type;
        $res1    = $this->PrescriptionModel->UserMedicine($user_id);
        if($res1!=false)
        {
            $result1=$res1->result();
            $data->prescription1=$result1;
            $this->load->view('UserMedicition',$data);
        }
        else
        {
            $data->error=1;
            $data->message="No Medicine Available";
            $this->load->view('UserMedicition',$data);
        }
    }
    ...
    public function MedicineHistory()
    {
        ...
        $data->page      =  $_GET['type'];
        $medicine_details = $this->PrescriptionModel->prescription_record($medicine_id,$prescription_id);
        $medicineTime = $this->PrescriptionModel->MedicineHistory($medicine_id);
        if(!empty($medicineTime))
        {            
            $data->MedDetails=$medicine_details;
            $data->medicineTime=$medicineTime;
            $this->load->view('MedicineHistory',$data);
        }
        else
        {
            $data->error=1;
